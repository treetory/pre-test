/*
Programmers
Joe, Cho edited this page 8 hours ago · 1 revision
문제 설명
...
function solution(progresses, speeds) {
    ...
    let test1 = progresses.map( )
    
    for(let i=0; i<progresses.length ;i++){
        let minusFromH = 100 - progresses[i];
        let day = minusFromH / speeds[i];
        temp.push(Math.ceil(day));
    }
...
function solve(p, s, acc) {
    if (p.every(v => v >= 100))
        return p;    
    p = zip(p, s);
    console.log(p);
    return solve(p, s, acc);
}

function drop(arr, length) {
    arr.splice(0, length);
    return arr;
}

function zip(p, s) {
    //return p.map((v, i, a) => (v < 100) ? a[i] = v + s[i] : a[i] );
